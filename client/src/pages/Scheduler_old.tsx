            ...
            <form className="space-y-5" onSubmit={handleSchedule}>
                ...
                <div className="flex flex-wrap gap-3">
                  {PLATFORMS.map((p)=>{
                    ...
                    return (
                      ...
                      className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-amber-50 border-amber-300 text-yellow-600 scale-103" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}>
              ...
              <div>
                ...
                <textarea required rows={5} placeholder="What do you want to share today?" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content} onChange={(e)=>setContent(e.target.value)}/>
                  <div className={`text-right text-xs mt-1 font-medium ${content.length > 270 ? "text-yellow-600" : "text-slate-400"}`}>
                    {content.length}/280
                  </div>
              </div>
              ...
              <div>
                ...
                {mediaFile ? (
                  ...
                  <label className="flex items-center justify-center gap-2 p-5 py-10 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-amber-300 hover:bg-amber-50/40 transition-all group">
                    <span className="text-sm text-slate-500 group-hover:text-yellow-700 transition-colors">Click to upload image or video</span>
              ...
              {/* Submit */}
              <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3.5 bg-yellow-500 hover:bg-yellow-600 transition-all text-white rounded-lg">
                {loading ? (
                  <>
