                ...
                {PLATFORMS.map((p)=>{
                    ...
                    return (
                        ...
                        className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-amber-300 bg-amber-50 cursor-default" : "border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}>
                            ...
                            {/* Icon */}
                            <div className="p-2">
                                <p.icon className={`size-5 ${isConnected ? "text-yellow-700" : "text-slate-500"}`}/>
                            </div>

                            {/* Label */}
                            <div className="flex-1 min-w-0">
                                <div className={`text-sm ${isConnected ? "text-yellow-800" : "text-slate-800"}`}>
                                    {p.name}
                                </div>
                              ...
                              {/* Status */}
                            {isConnected && <CheckCircleIcon className="size-4 text-yellow-600 shrink-0"/>}
                            {isConnecting && <div className="size-4 border-2 border-yellow-600 border-t-transparent rounded-full animate-spin shrink-0"/>}
